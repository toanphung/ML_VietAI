...
if __name__ == "__main__":
    ...
    w = tf.Variable(np.random.normal(0, np.sqrt(2. / np.sum(w_shape)), w_shape), name='weights')

    # [TODO 2.9] Create a feed-forward operator
    z = tf.matmul(x, w)
    z_prime = tf.exp(tf.subtract(z, tf.reshape(tf.reduce_max(z, axis=1), (tf.shape(x)[0], 1))))
    s = tf.reshape(tf.reduce_sum(z_prime, axis=1), (tf.shape(x)[0], 1))
    pred = z_prime/s

    # [TODO 2.10] Write the cost function
    cost = -(tf.reduce_sum(tf.reduce_sum(y*tf.log(pred), axis=1)))/train_x.shape[0]
