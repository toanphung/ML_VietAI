...
class SoftmaxClassifier(LogisticClassifier):
    ...
    def feed_forward(self, x):
        ...
        z, z_max = self.softmax(x)
        z_prime = np.exp(z-z_max)
        s = np.sum(z_prime, axis=1).reshape((z.shape[0], 1))
        return z_prime/s
    ...
    def compute_loss(self, y, y_hat):
        ...
        m = y.shape[0]
        cost = -np.sum(np.sum(y*np.log(y_hat), axis=1))/m
...
def normalize(train_x, val_x, test_x):
    ...
    m = train_x.shape[0]
    R = train_x.shape[1]
    train_mean = np.sum(np.sum(train_x))/(m*R)
    train_std = np.sqrt(np.sum(np.sum((train_x - train_mean)**2))/(m*R))
...
def create_one_hot(labels, num_k=10):
    ...
    temp = np.zeros((labels.shape[0], num_k))
    temp[np.arange(labels.shape[0]), labels] = 1
    return temp
...
def test(y_hat, test_y):
    ...
    y_hat_1d = np.where(y_hat == np.max(y_hat, axis= 1).reshape((y_hat.shape[0],1)))[1]
    test_y_1d = np.where(test_y == 1)[1]
    # [TODO 2.7]
    # Compute the confusion matrix here
    for i in range(test_y_1d.shape[0]):
        confusion_mat[test_y_1d[i], y_hat_1d[i]] += 1
...
if __name__ == "__main__":
    ...
    #     if len(all_val_loss) > 1 and all_val_loss[-1] > all_val_loss[-2]:
    #         count += 1
    #
    #     if count >= 10:
    #         break
    ...
    #
    # np.savetxt('./data/my_params_softmax_np', dec_classifier.w,
    #            header='Params was trained on %s' % str(datetime.datetime.now()))
    # Load 2 lines below when got theta trained
    params = np.loadtxt('./data/my_params_softmax_np')
    dec_classifier.w = params
