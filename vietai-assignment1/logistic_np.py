...
class LogisticClassifier(object):
    ...
    def feed_forward(self, x):
        ...
        result = 1/(1+np.e**(-np.dot(x, self.w)))
        return result
...
def normalize_per_pixel(train_x, test_x):
    ...
    # test_x = ...
    m = train_x.shape[0]
    train_mean = 1/m*np.sum(train_x, 0)  # shape=(64,64)
    train_std = np.sqrt(1/m*np.sum((train_x-train_mean)**2, 0))  # shape=(64,64)
    train_x = (train_x-train_mean)/train_std  # shape=(2400, 64, 64)
    test_x = (test_x-train_mean)/train_std  # shape=(600, 64, 64)
...
def normalize_all_pixel(train_x, test_x):
    ...
    # train_x = ...
    # test_x = ...
    m, R, C = train_x.shape[0], train_x.shape[1], train_x.shape[2]
    train_mean = 1/(m*C*R)*np.sum(np.sum(np.sum(train_x)))
    train_std = np.sqrt(1/(m*R*C)*np.sum(np.sum(np.sum((train_x - train_mean)**2))))
...
def test(y_hat, test_y):
    ...
    all = np.where(test_y == 1)[0].shape[0]
    # TP = np.where(y_hat > 0.5 and test_y == 1)[0].shape[0]
    TP = np.where(np.logical_and(y_hat > 0.5, test_y == 1))[0].shape[0]
    FP = np.where(np.logical_and(y_hat > 0.5, test_y == 0))[0].shape[0]
...
if __name__ == "__main__":
    ...
    np.savetxt('./data/my_params_logistic_np', bin_classifier.w, header='Params was trained on %s' % str(datetime.datetime.now()))
    # params = np.loadtxt('./data/my_params_logistic_np')
    # bin_classifier.w = params
