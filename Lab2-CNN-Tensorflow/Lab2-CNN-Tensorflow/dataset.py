...
class DataSet:
    def __init__(self):
        mnist = tf.contrib.learn.datasets.load_dataset("mnist") # down tren mang ve load vao memory
        self.train_data = mnist.train.images  # Returns np.array | shape=(55000 images, 28x28 pixels image)
        self.train_labels = np.asarray(mnist.train.labels, dtype=np.int32) # shape=(55000, 1) (0 --> 9)
        self.eval_data = mnist.test.images  # Returns np.array | shape = (1000 images, 28x28)
        self.eval_labels = np.asarray(mnist.test.labels, dtype=np.int32) # shape = (1000, 1)
